            ...
            <div class="card border-0 shadow">
                <div class="card-body">
                    <div class="d-flex justify-content-between align-items-center">
                        <div class="fs-4 fw-bold">{{ __('Experiences') }}</div>
                        <a href="{{ route('backend.experiences.create') }}" class="btn btn-primary">{{ __('Add New') }}</a>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <tbody>
                                @foreach ($experiences as $experience)
                                    <tr>
                                        ...
                                        <td class="text-center">{{ $experience->started_at->format('d-m-Y') }}</td>
                                        <td class="text-center">{{ $experience->end_at->format('d-m-Y') }}</td>
                                        <td class="text-center">{{ $experience->status }}</td>
                                        <td class="text-end">
                                            <div class="d-flex justify-content-end align-items-center gap-3">
                                                <a href="{{ route('backend.experiences.edit', $experience->id) }}" class="btn btn-primary btn-sm"><i class="ph-note-pencil fs-5"></i></a>

                                                <form method="POST" action="{{ route('backend.experiences.destroy', $experience->id) }}" onsubmit="return confirm('Are you sure?');">
                                                    @csrf
                                                    @method('DELETE')
